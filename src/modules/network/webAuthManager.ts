...
export class WebAuthManager implements WebAPI.Auth.IWebAuthManager {
    ...
    private lastFailure: WebAPI.Auth.ILastFailureReasonObject = {
        error: null,
        reason: "Success"
    }
     ...
     */
    private async _performQuery(queryStr: string, values: Array<string | number>, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Mysql.TGenericMysqlResult | null> {
        const connection = conn ?? await this.db.getConnection();

        if(connection) {
            return await new Promise<WebAPI.Mysql.TGenericMysqlResult | null>(res=>{
                connection.query(queryStr,values,(err: MysqlError | null,results: WebAPI.Mysql.TGenericMysqlResult)=>{
                    if(err) {
                        this.db.reportMysqlError(err);
                        this.lastFailure = {error: err, reason: "DBError"};
                        res(null);
                        return;
                    }
                    this.lastFailure = {error: null, reason: "Success"};
                    res(results);
                    if(!conn) connection.release();
                });
            });
        }else {
            this.lastFailure = {error: null, reason: "NoConnection"};
            return null;
        }
    }

     /**
     * Returns error of last _performQuery call
     * @throws Do not use if _performQuery returned positive response. Will throw.
     * @returns Mysql Error
     */
    private _getLastQueryError() {
        if(this.lastFailure.error)
            return this.lastFailure.error;
        else throw new Error("[AuthManager] Trying to read last error on successfull request result.");
    }

    /**
     * Returns failure reason of last _performQuery call
     * @throws Do not use if _performQuery returned positive response. Will throw.
     * @returns Failure reason
     */
    private _getLastQueryFailureReason() {
        if(this.lastFailure.reason!="Success")
            return this.lastFailure.reason;
        else throw new Error("[AuthManager] Trying to read last failure reason on successfull request result.");
    }
    ...
    public async tryLogin(email: string, password: string,ipAddress: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.SessionAPI.TLoginResult> {
        ...
        if(connection) {
            ...
            if(user.result=="Success"&&(await compare(password, user.data.password))) {
                ...
                let response = await this._performQuery(queryStr,[newSessionID, user.data.userID, ipAddress, DateTime.now().plus({days: 7}).toISO()]);
                if(response) {
                    if((response as WebAPI.Mysql.IMysqlQueryResult).affectedRows===1) {
                        queryStr = `UPDATE users SET lastAccessDate=NOW() WHERE userID=?`;
                        response = await this._performQuery(queryStr,[user.data.userID],connection);
                        if(response) {
                            result = {
                                result: "Success",
                                data: newSessionID
                            ...
                            connection.release();
                            return result;
                        }else result.result = this._getLastQueryFailureReason();
    ...
    public async isSessionValid(sessionToken: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.SessionAPI.TGetSessionValidResult> {
        const result = await this._performQuery(`SELECT userID FROM auth_sessions WHERE sessionID=? AND now() < expirationDate`,[sessionToken], conn);

        if(result) {
            return (result as WebAPI.Mysql.TMysqlSelectQueryResult).length===1;
        }else return this._getLastQueryFailureReason();
    }
    ...
    public async prolongSession(sessionToken: string, ipAddress: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.SessionAPI.TProlongSessionResult> {
        ...
        if(connection) {
            ...
            if(session.result=="Success") {
                ...
                let response = await this._performQuery(queryStr,[ipAddress,sessionToken],conn);
    
                if(response) {
                    if((response as WebAPI.Mysql.IMysqlQueryResult).affectedRows===1) {
                        queryStr = `UPDATE users SET lastAccessDate=NOW() WHERE userID=?`;     

                        response = await this._performQuery(queryStr,[session.data.userID]);

                        if(response) {
                            connection.commit();
                            connection.release();
                            return true;
                        }else result = this._getLastQueryFailureReason();
    ...
    public async getSessionDetails(sessionToken: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.SessionAPI.TGetSessionDetailsResult> {
        ...
        const response = await this._performQuery(queryStr,[sessionToken],conn);

        if(response) {
            const rows = (response as WebAPI.Mysql.TMysqlSelectQueryResult);
            if(rows.length===1) {
                result = {
                    result: "Success",
                    data: {
                        ...
                        creationDate: DateTime.fromISO(rows[0]["creationDate"]),
                        lastAccessDate: DateTime.fromISO(rows[0]["lastAccessDate"]),
                        expirationDate: DateTime.fromISO(rows[0]["expirationDate"])
                    }
                }
            }
        }else result.result = this._getLastQueryFailureReason();
    ...
    public async dropSession(sessionToken: string, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.SessionAPI.TDropSessionResult> {
        const result = await this._performQuery(`DELETE FROM auth_sessions WHERE sessionID=?`,[sessionToken],conn);

        if(result) {
            if((result as WebAPI.Mysql.IMysqlQueryResult).affectedRows===1) {
                return true;
    ...
    public async getAllSessions(userID: number, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.SessionAPI.TGetAllSessionsResult> {
        ...
        if(connection) {
            ...
            const response = await this._performQuery(query,[],connection);
    ...
    public async dropAllExpiredSessions(conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.TGenericActionResult> {
        const response = await this._performQuery(`DELETE FROM auth_sessions WHERE now() > expirationDate;`,[],conn);    

        if(response) {
            return true;
        }return this._getLastQueryFailureReason();
    }
        ...
        if(connection) {
            if(emailRegex.test(userData.email)) {
                ...
                if(user.result!="Success") {
                    ...
                    const response = await this._performQuery(queryStr, [userData.email.toLowerCase(), userData.name, userData.surname, userData.gender], connection);
    ...
    public async userExists(email: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.UserAPI.TUserExistsResult> {
        const result = await this._performQuery(`SELECT userID FROM users WHERE email=?;`,[email.toLowerCase()],conn);
    ...
    public async getUser(userKey: string | number, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.UserAPI.TGetUserResult> {
        ...
        const response = await this._performQuery(`SELECT *, DATE_FORMAT(creationDate, '%Y-%m-%dT%TZ') as creationDate, DATE_FORMAT(lastAccessDate, '%Y-%m-%dT%TZ') as lastAccessDate, DATE_FORMAT(lastPasswordChangeDate, '%Y-%m-%dT%TZ') as lastPasswordChangeDate FROM users NATURAL JOIN roles WHERE ${field}=?;`,values,conn);
            ...
            if(rows.length===1) {
                result = {
                    ...
                    data: {
                        ...
                        roleName: rows[0]["friendlyName"],
                        creationDate: DateTime.fromISO(rows[0]["creationDate"]),
                        lastAccessDate: DateTime.fromISO(rows[0]["lastAccessDate"]),
                        lastPasswordChangeDate: DateTime.fromISO(rows[0]["lastPasswordChangeDate"])
                    }
                }
    ...
    public async setPassword(userKey: string | number, newPassword: string, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.UserAPI.TSetPasswordResult> {
        ...
        const result = await this._performQuery(`UPDATE users SET password=?, lastPasswordChangeDate=NOW() WHERE ${field}=?`,[hashedPassword, userKey],conn);

        if(result) {
            if((result as WebAPI.Mysql.IMysqlQueryResult).changedRows===1) return true;
    ...
    public async createToken(actionName: WebAPI.Auth.AccountsTokenAPI.TAccountActionName ,userKey: string | number, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.AccountsTokenAPI.TCreateTokenResult> {
        ...
        if(userID!=undefined) {
            ...
            if(connection) {
                ...
                let response = await this._performQuery("SELECT accountActionTypeID FROM account_action_types WHERE accountActionName=?",[actionName],connection);
                const actionTypeID = response?(response as WebAPI.Mysql.TMysqlSelectQueryResult)[0]["accountActionTypeID"]:-1;
                ...
                if(actionTypeID!=-1) {
                    ...
                    if(tokenCount.result=="Success") {
                        if(tokenCount.data < 7) {
                            ...
                            const response = await this._performQuery(queryStr,[token,actionTypeID,userID, DateTime.now().plus({hours: 24}).toISO()],connection);
    ...
    public async getTokenDetails(token: string, conn?: WebAPI.Mysql.IPoolConnection): Promise<WebAPI.Auth.AccountsTokenAPI.TGetTokenDetailsResult> {
        ...
        const response = await this._performQuery(`SELECT *, DATE_FORMAT(creationDate, '%Y-%m-%dT%TZ') as creationDate, DATE_FORMAT(expirationDate, '%Y-%m-%dT%TZ') as expirationDate FROM account_actions NATURAL JOIN account_action_types WHERE accountActionTokenID=? AND now() < expirationDate;`,[token],conn);
        ...
        if(response) {
            ...
            if(rows.length===1) {
                result = {
                    ...
                    data: {
                        tokenID: token,
                        actionTypeID: rows[0]["accountActionTypeID"],
                        actionTypeName: rows[0]["accountActionName"],
                        userID: rows[0]["userID"],
                        creationDate: DateTime.fromISO(rows[0]["creationDate"]),
                        expirationDate: DateTime.fromISO(rows[0]["expirationDate"])
                    }
    ...
    public async getTokensCount(userKey: string | number, actionName: WebAPI.Auth.AccountsTokenAPI.TAccountActionName, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.AccountsTokenAPI.TGetTokenCountResult> {
        ...
        if(user.result=="Success") {
            const response = await this._performQuery(`SELECT COUNT(*) as count FROM account_actions NATURAL JOIN account_action_types WHERE userID=? AND accountActionName=? AND now() < expirationDate`,[user.data.userID,actionName],conn);
            
            if(response) {
                const row0 = (response as WebAPI.Mysql.TMysqlSelectQueryResult)[0];
                result = {
                    result: "Success",
                    data: row0["count"]
                }
    ...
    public async dropToken(token: string, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.AccountsTokenAPI.TDropTokenResult> {
        const result = await this._performQuery(`DELETE FROM account_actions WHERE accountActionTokenID=?`,[token],conn);

        if(result) {
            if((result as WebAPI.Mysql.IMysqlQueryResult).affectedRows===1) return true;
    ...
    public async dropAllExpiredTokens(conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.TGenericActionResult> {
        const response = await this._performQuery(`DELETE FROM account_actions WHERE now() > expirationDate;`,[],conn);    
    ...
    public async getTokenTypes(conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.AccountsTokenAPI.TGetTokenTypesResult> {
        ...
        if(connection) {
            const response = await this._performQuery(`SELECT accountActionName FROM account_action_types;`,[],connection);
            ...
            if(connection) {
                ...
                if(!userExists) {
                    ...
                    if(tokenDetails.result=="InvalidToken") {
                        ...
                        const response = await this._performQuery(`INSERT INTO invites(inviteTokenID, email, expirationDate) VALUES(?,?,?)`,[token,email.toLowerCase(), expirationDateStr],conn);
    ...
    public async getInviteDetails(query: string, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.InviteAPI.TGetInviteDetailsResult> {
        ...
        const response = await this._performQuery(queryStr,[query.toLowerCase()],conn);
    ...
    public async dropInvite(token: string, conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.InviteAPI.TDropInviteResult> {
        const result = await this._performQuery(`DELETE FROM invites WHERE inviteTokenID=?`,[token],conn);
    ...
    public async getAllInvites(conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.InviteAPI.TGetAllTokensResult> {
        ...
        const response = await this._performQuery(query,[],conn);
    ...
    public async dropAllExpiredInvites(conn?: WebAPI.Mysql.IPoolConnection | undefined): Promise<WebAPI.Auth.TGenericActionResult> {
        const response = await this._performQuery(`DELETE FROM invites WHERE now() > expirationDate;`,[],conn);    
